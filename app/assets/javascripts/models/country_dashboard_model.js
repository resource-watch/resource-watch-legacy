(function(App) {
  ...
  App.Model.CountryDashboard = App.Core.Model.extend({
    ...
    linesParser: function(data, configuration) {
      ...
      /* We prefer to show "no data available" when we just have data for one
       * point (and thus we can't draw any line) */
      if(values.length === 1) {
        return this.noneParser(data, configuration);
      }
